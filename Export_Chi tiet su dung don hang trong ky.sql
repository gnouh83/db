SELECT TO_CHAR(TRUNC(ADD_MONTHS(SYSDATE, -1), 'MM'), 'mm/yyyy') thang,
       ...
       z.not_yet_ton                                            sl_ton,
       z.taomoi                                                 sl_taomoi,
       z.used_in_period                                         sl_tieudung,
       z.not_yet                                                sl_cuoiky,
       z.expired_in_period                                      sl_hethan
FROM (SELECT (SELECT com_name FROM dsp_company WHERE com_id = dt.top_id) ctkv,
             ...
             NVL((SELECT b.amount
                  FROM dsp_dc_detail b
                  WHERE b.transaction_id = t.transaction_id
                    AND t.status IN (3, 6)
                    AND b.card_name = o.profile_code
                    AND b.request_time >= TRUNC(ADD_MONTHS(SYSDATE, -1), 'MM')
                    AND b.request_time < TRUNC(SYSDATE, 'MM')), 0)       taomoi
